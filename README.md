...
[![Linux Build and Test](https://github.com/Dingola/QtRecordParser/actions/workflows/build_and_test_linux.yml/badge.svg)](https://github.com/Dingola/QtRecordParser/actions/workflows/build_and_test_linux.yml)
[![macOS Build and Test](https://github.com/Dingola/QtRecordParser/actions/workflows/build_and_test_macos.yml/badge.svg)](https://github.com/Dingola/QtRecordParser/actions/workflows/build_and_test_macos.yml)
[![Windows Build and Test](https://github.com/Dingola/QtRecordParser/actions/workflows/build_and_test_windows.yml/badge.svg)](https://github.com/Dingola/QtRecordParser/actions/workflows/build_and_test_windows.yml)

### Code Coverage

[![codecov](https://codecov.io/gh/Dingola/QtRecordParser/graph/badge.svg)](https://codecov.io/gh/Dingola/QtRecordParser)
...
|-----------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------|
| **Sunburst** | The inner-most circle represents the entire project. Moving outward are folders, and finally individual files. The size and color of each slice represent the number of statements and the coverage, respectively. | <img src="https://codecov.io/gh/Dingola/QtRecordParser/graphs/sunburst.svg" alt="Sunburst" height="100" width="200"> |
| **Grid**     | Each block represents a single file in the project. The size and color of each block represent the number of statements and the coverage, respectively.                                                                      | <img src="https://codecov.io/gh/Dingola/QtRecordParser/graphs/tree.svg" alt="Grid" height="100" width="200">         |
| **Icicle**   | The top section represents the entire project, followed by folders and individual files. The size and color of each slice represent the number of statements and the coverage, respectively.                                 | <img src="https://codecov.io/gh/Dingola/QtRecordParser/graphs/icicle.svg" alt="Icicle" height="150" width="400">     |
...
### 2) Setting up
```
git clone https://github.com/Dingola/QtRecordParser.git
cd QtRecordParser/
```
<br>
...
This project builds a library and therefore does not produce a directly runnable
application. Link the generated library from a consumer target or run its test target.
...
```
docker build -t qtrecordparser-dockerimage .
```
<br>
...
- **Run directly:**
```
docker run qtrecordparser-dockerimage
```
- **Start an interactive Bash shell:**
```
docker run -it qtrecordparser-dockerimage bash
```
<br>
...
```
Xvfb :99 -screen 0 1920x1080x24 -nolisten tcp & export DISPLAY=:99 && ./_build_tests_release/QT_Project_Tests/QtRecordParser_Tests
```
<br>
...
```
docker run -it --name qtrecordparser-container --network host -e DISPLAY=<IP-ADDRESS>:99.0 -e TERM=xterm-256color -e QT_X11_NO_MITSHM=1 qtrecordparser-dockerimage bash
```
> [!NOTE]
...
- **Run the tests:**
```
"./_build_tests_release/QT_Project_Tests/QtRecordParser_Tests"
```
<br>
